...
public class FileService {
	 ...
	 * 
	 * * Java는 모든 운영체제에서 호환된다!!!
	 * 	-> 코드에서 경로 표기식 \,/를 신경쓰지 않아도 된다.
	...
	public void test1() {
		...
		if (!directory.exists()) { // 폴더가 없다면
			...
			File parent = new File(directory.getParent()); // 무모 폴더 참조
	...
	public void test3() {
		...
		for(File f : files) {
			...
			// -> 1970.01.01 09:00:00.000 기준으로
			// 	  지난 시간 만큼의 ms 값이 반환
			
			// SimpleDateFormat : 간단히 날자 형식을 지정할 수 있는 객체
