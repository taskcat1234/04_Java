...
public class ArrayService1 {
	...
	public void method1() {
		...
		// int[] 자료형을 저장화기 위한 공간(변수)을 메모리에 할당하고
		...
		// arr = new int[4];
		// -> heap 메모리 영역에 생성된 int 4칸 짜리 배열의
	...
	public void method4() {
		...
		// -> 웹과 다르게 자바 배열은 존재하지 않는 인덱스에 접근 불가!!
		// -> 처음 배열이 할당될 때의 크기로 고정된다!!
		
		System.out.print("확인  : " + arr[3]);
	}
	...
	public void method5() {
		...
		// 0번은 비교할 필요가 없으니 1번 부터 시작 (0번은 기준값)
		for(int i = 1 ; i < scoreArr.length ; i++) {
			...
			// 최대값 찾기
			if (scoreArr[i] < scoreArr[manIndex]) {
				manIndex = i; // 현재 인덱스 i를 manIndex에 대입
			}
		}
		System.out.println("최대값 : " + scoreArr[maxIndex]);
		System.out.println("최소값 : " + scoreArr[manIndex]);
