...
public class AbstractionRun {
	public static void main(String[] atgs) {
		...
		// 그리고 실제 실행 시 Car eat 실행됨 (동적 바인딩)
		System.out.println("--------------------------------------");
		a2.eat();
		a2.move();
		a2.sleep();
		...
		System.out.println("--------------------------------------");
		/* 확인 4. 추상 클래스 활용법 */
		printAnimal(new Cat("포유류","털없음"));
		printAnimal(new Snake("파충류"));
		
	} // main end
	
	// static : 프로그램 실행 시 해석되어
	// 					static 메모리 영역에 생성되는 키워드
	//          -> 객체 생성 없이 클래스명.메서드명 호출 가능
	// 					+ 같은 클래스라도 static 메서드 내에서 다른 메서드를 호출할려면
	// 				  1) 현재 클래스를 이용해서 객체 생성
	...
	public static void printAnimal(Animal animal) {
		...
		// - Animal을 상속 받은 클래스는
		//   반드시 상속 받아 재정의한 eat(), move(), sleep() 존재
		//   -> 자식에게 고통적인 규약 설정(오버라이딩 강제화)
