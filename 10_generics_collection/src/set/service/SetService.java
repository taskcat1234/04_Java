...
public class SetService {
	 ...
	 * set 인터페이스를 구현한 구현체
	 * 
	 * 1. Hashset : 처리 속도가 빠른 Set (제일 많이 사용 대표적임)
	 * 
	 * 2. LinkedHashset : 순서를 유지하는 Set ( 순서를 유지하고 싶을때 사용 )
	 * 
	...
	public void test1() {
		...
		/* String toString() : 저장된 값을 문다열로 출력*/
		// -> 순서 유지 x 확인
		...
		System.out.println("쿠팡 제거됨? " + set.remove("삼성"));
		System.out.println(set); // 쿠팡 지워 졌는지? 지워짐
	...
	public void test2() {
		...
		Set<Person> personSet = new LinkedHashSet<Person>(); // 넣은 순서대로 출력할시
		...
		// 2. equals() 오버라이딩 0, hashCode() 오버라이딩 x
		// -> 중복 제거 안됨
		
		// 3. equals() 오버라이딩 x, hashCode() 오버라이딩 0
	  // -> 중복 제거 안됨
		
		// 4. equals() 오버라이딩 0, hashCode() 오버라이딩 0
		// -> 중복 데이터 제거 성공!!!
	...
	public void test3() {
		...
		for(String snack : snacks) {
			System.out.println(snack);  //위에껄 단순화 시킨 버전
		}
		...
		// -> snacks Set에 저장된 데이터를 이용해 ArrayList를 생성함
	...
	public void test4() {
		//Set<String> set = new HashSet<String>();
		Set<String> set = new TreeSet<String>(); // 오름차순 정렬 ㄱ,ㄴ,ㄷ,ㄹ,ㅁ 순
	...
	// 로또 번호 6개(중복 X, 오름차순 정렬)가 담긴 문자열 반환
// 밑에는 기존 방법
...
//	}
	...
	public void lottoNumberGenrator() {
		...
		System.out.println("생성할 로또 번호 개수 입력 : ");
