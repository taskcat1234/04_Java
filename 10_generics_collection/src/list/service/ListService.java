...
public class ListService {
	 ...
	 * 	== 순서가 있음
	 * 
	 * - index로 데이터 구분되기 때문에 중복 값 저장 가능
	 */
	
	public void test1() {
		
		/* List는 인터페이스!! -> 직접 객체 생성 불가
		 * --> 상속 받아 구현한 자식 클래스를 이용
		 * 
		 * [ArrayList]
		 * - 배열 처럼 생긴 List
		 */
	...
	public void test2() {
		 ...
		 * - 결과로 이전에 존재하던 요소(E)를 반환받는다
		 */
		String befString = menuList.set(2, "불족발");
		System.out.println("-----------------------------");
		System.out.println(befString + "이/가 수정 되었습니다");
		System.out.println(menuList);
		
		/* boolean contains(E e) 
		 * - 같은 객체가 있는지 확인 (있으면 true, 없으면 false)
		 * - 비교할려는 객체가 equals()를 오버라이딩 해야함 
		 */
		 ...
		 * - index번째 요소 삭제
		 * - 삭제된 요소(E) 반환됨
		 * - 중간 요소가 삭제되면 알아서 빈공간을 채움(앞으로 당기기)
		 */
		System.out.println("-------------------------------------------------------");
		System.out.println("보쌈 인덱스 : " + menuList.indexOf("보쌈"));
...
//		}
		...
		for(int i = 0 ; i < 1_000_000 ; i++) { // 100만 바퀴
...
//		tist3.addAll(temp)); // String은 Parent 상속 관계 X => 오류
		for(Parent p : list3) {
